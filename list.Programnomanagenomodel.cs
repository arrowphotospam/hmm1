    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                else if (choice == "5") foreach (var c in contacts.OrderBy(x => x.ContactId)) c.DisplayContactDetails();
                else if (choice == "6") foreach (var c in contacts.OrderBy(x => x.Name)) c.DisplayContactDetails();
                else if (choice == "7")
                {
                    int id = int.Parse(Console.ReadLine());
                    var c = contacts.FirstOrDefault(x => x.ContactId == id);
